...
const limiter = new Bottleneck({ minTime: 200 }); // ~5 req/s ≈ 300/min
const retry = new RetryStrategy({ initialDelayMs: 1000, maxAttempts: 7 });

type FortnoxOptions = { baseUrl?: string };
export class FortnoxClient {
  private integrationId: string;
  private baseUrl: string;

  constructor(integrationId: string, opts: FortnoxOptions = {}) {
    this.integrationId = integrationId;
    this.baseUrl = opts.baseUrl || process.env.FORTNOX_BASE_URL || 'https://api.fortnox.se/3';
  }
    ...
    return limiter.schedule(() =>
      retry.execute<T>(async () => {
        ...
        if (!res.ok) {
          ...
          const err: any = new Error(`Fortnox API ${res.status}: ${text}`);
  ...
  // ---- Projects / Employees / Time ---- (stubbar - fyll på efter docs)
  createProject(payload: any) { return this.request('/projects', { method: 'POST', body: JSON.stringify(payload) }); }
  createEmployee(payload: any) { return this.request('/employees', { method: 'POST', body: JSON.stringify(payload) }); }
  exportTimeEntry(payload: any) { return this.request('/time/entries', { method: 'POST', body: JSON.stringify(payload) }); }
}
